...
(function(b, c) {
        ...
        if (this.containers.length === 1) {
          ...
          if (this.currentContainer != this.containers[m]) {
            ...
            for (var g = this.items.length - 1; g >= 0; g--) {
              ...
              if ((d.pageX < this.items[g].item.offset().left || d.pageX > this.items[g].item.offset().left + this.items[g].item.outerWidth()) || (d.pageY < this.items[g].item.offset().top || d.pageY > this.items[g].item.offset().top + this.items[g].item.outerHeight())) {
                continue
              }
              if (this.currentItem[0] == this.items[g].item[0]) {
                continue
              }
    ...
    b.ui.plugin.add("sortable", "smartguides", {
      start: function(g, h) {
        var e = b(this).data("sortable")
          , k = e.options
          , j = k.tolerance;
        e.elements = [];
        e.guidesElements = [];
        Fai.top.$("#g_main").off("scroll.sort").on("scroll.sort", function() {
          f()
        });
        function f() {
          e.elements = [];
          var l = b(window)
            ...
            , n = l.width()
            , i = Fai.isIE8();
          b(k.smartguides.constructor != String ? (k.smartguides.items || ":data(sortable") : k.smartguides).each(function() {
            var s = b(this)
              , v = s.offset()
              , x = this.getBoundingClientRect()
              , u = i ? (x.bottom - x.top) : x.height
              , z = i ? (x.right - x.left) : x.width
              , A = v.top
              , y = A + u
              , q = v.left
              , p = z + q;
      ...
      stop: function() {
        b("#absAlignTop, #absAlignRight, #absAlignBottom, #absAlignLeft, #absAlignLevel, #absAlignVertical").hide();
        b("#g_main").off("scroll.sort")
      },
      sort: function(U, F) {
        var D = b(this).data("sortable")
          , J = D.options
          , Q = J.smartTolerance
          , E = J.scale || 1;
        b("#absAlignTop, #absAlignRight, #absAlignBottom, #absAlignLeft, #absAlignLevel, #absAlignVertical").hide();
        var m = F.helper.is(":visible") ? F.helper[0] : b(".moduleSortableHelper")[0];
        if (!m) {
          return
        }
        var s = m.getBoundingClientRect();
        var y = Fai.isIE8()
          , u = F.offset.left + D.margins.left
          , e = y ? (s.right - s.left) : s.width
          , q = u + e
          , n = (u + q) / 2
          , W = F.offset.top + D.margins.left
          , f = y ? (s.bottom - s.top) : s.height
          , V = W + f
          , T = (W + V) / 2;
        var g = [W, V]
          , S = [W, V]
          ...
          , A = 0
          , H = 0;
        for (var O = D.elements.length - 1; O >= 0; O--) {
          var K = D.elements[O]
            , M = K.left
            , I = M + K.width
            , G = K.top
          ...
          var z = {
            ...
            bs2: Math.abs(R - V) <= Q,
            bs3: Math.abs(R - T) <= Q,
            levelS: Math.abs(h - W) <= Q,
            levelS2: Math.abs(h - V) <= Q,
            levelS3: Math.abs(h - T) <= Q,
            verticalS: Math.abs(x - u) <= Q,
            verticalS2: Math.abs(x - q) <= Q,
            verticalS3: Math.abs(x - n) <= Q
          };
          B(z)
        }
        function B(Z) {
          var aa = Z.type
            ...
            , ai = Z.verticalS3;
          if (af) {
            F.position.left = D._convertPositionTo("relative", {
              top: 0,
              left: an
            }).left - D.margins.left;
            g.push(ah, at);
          ...
          if (d) {
            F.position.top = D._convertPositionTo("relative", {
              top: ah,
              left: 0
            }).top - D.margins.top;
            N.push(an, aj);
        ...
        var L = Site.absAlignLine.commFunc;
        if (S.length > 2) {
          L.calLineSize("right", S);
          L.setAbsAlignLine("Right", C)
        }
        if (g.length > 2) {
          L.calLineSize("left", g);
          L.setAbsAlignLine("Left", A)
        }
        if (v.length > 2) {
          L.calLineSize("bottom", v);
          L.setAbsAlignLine("Bottom", j)
        }
        if (N.length > 2) {
          L.calLineSize("top", N);
          L.setAbsAlignLine("Top", w)
        }
        if (p.length > 2) {
          L.calLineSize("level", p);
          L.setAbsAlignLine("Level", P)
        }
        if (k.length > 2) {
          L.calLineSize("vertical", k);
          L.setAbsAlignLine("Vertical", H)
        }
      }
    })
  }
...
(function(b, c) {
    b.widget("ui.draggable", b.ui.mouse, {
      ...
      refreshConnectToSortable: function(d, f) {
        if (!this.element.data("draggable")) {
          return
        }
        var e = this.element.data("draggable")
          , g = e.options;
        e.sortables = [];
    ...
    b.ui.plugin.add("draggable", "smartguides", {
      ...
      drag: function(T, E) {
        var C = b(this).data("draggable")
          ...
          , D = I.scale || 1;
        b("#absAlignTop, #absAlignRight, #absAlignBottom, #absAlignLeft, #absAlignLevel, #absAlignVertical").hide();
        var q = E.helper[0].getBoundingClientRect();
        var x = Fai.isIE8()
          , s = E.offset.left + C.margins.left
          , e = x ? (q.right - q.left) : q.width
          , p = s + e
          , m = (s + p) / 2
          , V = E.offset.top + C.margins.left
          , f = x ? (q.bottom - q.top) : q.height
          , U = V + f
          , S = (V + U) / 2;
        var g = [V, U]
          , R = [V, U]
          ...
          , z = 0
          , G = 0;
        for (var N = C.elements.length - 1; N >= 0; N--) {
          var J = C.elements[N]
            , L = J.left
            , H = L + J.width
            , F = J.top
          ...
          var y = {
            ...
            bs2: Math.abs(Q - U) <= P,
            bs3: Math.abs(Q - S) <= P,
            levelS: Math.abs(h - V) <= P,
            levelS2: Math.abs(h - U) <= P,
            levelS3: Math.abs(h - S) <= P,
            verticalS: Math.abs(w - s) <= P,
            verticalS2: Math.abs(w - p) <= P,
            verticalS3: Math.abs(w - m) <= P
          };
          A(y)
        }
        function A(Y) {
          var Z = Y.type
            ...
            , ah = Y.verticalS3;
          if (ae) {
            E.position.left = C._convertPositionTo("relative", {
              top: 0,
              left: am
            }).left - C.margins.left;
            g.push(ag, ar);
          ...
          if (d) {
            E.position.top = C._convertPositionTo("relative", {
              top: ag,
              left: 0
            }).top - C.margins.top;
            M.push(am, ai);
        ...
        var K = Site.absAlignLine.commFunc;
        if (R.length > 2) {
          K.calLineSize("right", R);
          K.setAbsAlignLine("Right", B)
        }
        if (g.length > 2) {
          K.calLineSize("left", g);
          K.setAbsAlignLine("Left", z)
        }
        if (u.length > 2) {
          K.calLineSize("bottom", u);
          K.setAbsAlignLine("Bottom", j)
        }
        if (M.length > 2) {
          K.calLineSize("top", M);
          K.setAbsAlignLine("Top", v)
        }
        if (n.length > 2) {
          K.calLineSize("level", n);
          K.setAbsAlignLine("Level", O)
        }
        if (k.length > 2) {
          K.calLineSize("vertical", k);
          K.setAbsAlignLine("Vertical", G)
        }
      }
    });
...
(function(h, i) {
  ...
  h.fn.position = function(k) {
    ...
    k = h.extend({}, k);
    var o = h(k.of), n = o[0], q = (k.collision || "flip").split(" "), p = k.offset ? k.offset.split(" ") : [0, 0], m, j, l;
    if (n.nodeType === 9) {
      m = o.width();
      j = o.height();
      l = {
    ...
    l.top += p[1];
    return this.each(function() {
      var u = h(this), w = u.outerWidth(), t = u.outerHeight(), v = parseInt(h.curCSS(this, "marginLeft", true)) || 0, s = parseInt(h.curCSS(this, "marginTop", true)) || 0, y = w + v + (parseInt(h.curCSS(this, "marginRight", true)) || 0), z = t + s + (parseInt(h.curCSS(this, "marginBottom", true)) || 0), x = h.extend({}, l), r;
      if (k.my[0] === "right") {
        x.left -= w
      } else {
        if (k.my[0] === b) {
      ...
      _mouseStop: function(i) {
        ...
        if (this._helper) {
          ...
          var p = {
            ...
            , k = (parseInt(n.element.css("left"), 10) + (n.position.left - n.originalPosition.left))
            , m = (parseInt(n.element.css("top"), 10) + (n.position.top - n.originalPosition.top));
          if (j.reserveZero) {
            k = k === 0 || k ? k : null;
            m = m === 0 || m ? m : null
          } else {
...
(function(f, c) {
    ...
    function g() {
      ...
      this.dpDiv = b(f('<div id="' + this._mainDivId + '" class="ui-datepicker fk-commDatePicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>'))
    }
    f.extend(g.prototype, {
      ...
      _newInst: function(l, k) {
        ...
        return {
          ...
          drawYear: 0,
          inline: k,
          dpDiv: (!k ? this.dpDiv : b(f('<div class="' + this._inlineClass + ' ui-datepicker fk-commDatePicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>')))
        }
      },
      ...
      _showDatepicker: function(t) {
        ...
        if (t.nodeName.toLowerCase() != "input") {
          t = f("input", t.parentNode)[0]
        }
        if (f.datepicker._isDisabledDatepicker(t) || f.datepicker._lastInput == t) {
          return
        }
        var r = f.datepicker._getInst(t);
        if (f.datepicker._curInst && f.datepicker._curInst != r) {
          f.datepicker._curInst.dpDiv.stop(true, true);
          if (r && f.datepicker._datepickerShowing) {
            f.datepicker._hideDatepicker(f.datepicker._curInst.input[0])
          }
        }
        var o = f.datepicker._get(r, "beforeShow");
        var s = o ? o.apply(t, [t, r]) : {};
        if (s === false) {
          return
        }
        e(r.settings, s);
        r.lastVal = null;
        f.datepicker._lastInput = t;
        f.datepicker._setDateFromField(r);
        if (f.datepicker._inDialog) {
          t.value = ""
        }
        if (!f.datepicker._pos) {
          f.datepicker._pos = f.datepicker._findPos(t);
          f.datepicker._pos[1] += t.offsetHeight
        }
        var n = false;
        f(t).parents().each(function() {
          n |= f(this).css("position") == "fixed";
          return !n
        });
        if (n && f.browser.opera) {
        ...
        f.datepicker._updateDatepicker(r);
        q = f.datepicker._checkOffset(r, q, n);
        r.dpDiv.css({
        ...
        if (!r.inline) {
          var k = f.datepicker._get(r, "showAnim");
          var p = f.datepicker._get(r, "duration");
          var l = function() {
            var x = r.dpDiv.find("iframe.ui-datepicker-cover");
            if (!!x.length) {
              var w = f.datepicker._getBorders(r.dpDiv);
              x.css({
                left: -w[0],
                top: -w[1],
                width: r.dpDiv.outerWidth(),
                height: r.dpDiv.outerHeight()
              })
            }
          };
          var m = r.settings.zIndex || f(t).zIndex() + 1;
          r.dpDiv.zIndex(m);
          f.datepicker._datepickerShowing = true;
          if (f.effects && f.effects[k]) {
            r.dpDiv.show(k, f.datepicker._get(r, "showOptions"), p, l)
          } else {
            r.dpDiv[k || "show"]((k ? p : null), l)
          }
          if (!k || !p) {
            l()
          }
          if (r.input.is(":visible") && !r.input.is(":disabled")) {
            r.input.focus()
          }
          f.datepicker._curInst = r
        }
        var u = f.datepicker._get(r, "afterShow");
        var v = u ? u.apply(t, [t, r]) : {}
      },
      _updateDatepicker: function(m) {
        var q = this;
        q.maxRows = 4;
        var n = f.datepicker._getBorders(m.dpDiv);
        d = m;
        m.dpDiv.empty().append(this._generateHTML(m));
        this._attachHandlers(m);
        var r = m.dpDiv.find("iframe.ui-datepicker-cover");
        if (!!r.length) {
          r.css({
            left: -n[0],
            top: -n[1],
            width: m.dpDiv.outerWidth(),
            height: m.dpDiv.outerHeight()
          })
        }
        m.dpDiv.find("." + this._dayOverClass + " a").mouseover();
        var s = this._getNumberOfMonths(m);
        var o = s[1];
        var k = 17;
        m.dpDiv.removeClass("ui-datepicker-multi-2 ui-datepicker-multi-3 ui-datepicker-multi-4").width("");
        if (o > 1) {
          m.dpDiv.addClass("ui-datepicker-multi-" + o).css("width", (k * o) + "em")
        }
        m.dpDiv[(s[0] != 1 || s[1] != 1 ? "add" : "remove") + "Class"]("ui-datepicker-multi");
        m.dpDiv[(this._get(m, "isRTL") ? "add" : "remove") + "Class"]("ui-datepicker-rtl");
        if (m == f.datepicker._curInst && f.datepicker._datepickerShowing && m.input && m.input.is(":visible") && !m.input.is(":disabled") && m.input[0] != document.activeElement) {
          m.input.focus()
        }
        if (m.yearshtml) {
          var l = m.yearshtml;
          setTimeout(function() {
            if (l === m.yearshtml && m.yearshtml) {
              m.dpDiv.find("select.ui-datepicker-year:first").replaceWith(m.yearshtml)
            }
            l = m.yearshtml = null
          }, 0)
        }
        var p = this._get(m, "onAfterUpdatePicker");
        if (p) {
          p.apply((m.input ? m.input[0] : null), [m])
        }
      },
      ...
      _checkOffset: function(p, n, m) {
        ...
        if (n.top + s > l && l > s) {
          p.dpDiv.removeClass("f-onBelowInput").addClass("f-onTopInput")
        } else {
          p.dpDiv.removeClass("f-onTopInput").addClass("f-onBelowInput")
        }
      ...
      _adjustDate: function(p, o, n) {
        ...
        var k = this._get(l, "onAfterAdjustDate");
      ...
      _selectToMonthYearDate: function(p, l, n, k, o) {
        var m = f(p);
        l.selectedMonth = l.drawMonth = parseInt(n, 10);
        l.selectedYear = l.drawYear = parseInt(k, 10);
        l.selectedDay = parseInt(o, 10);
        this._notifyChange(l);
        this._adjustDate(m)
      },
      ...
      _selectDate: function(p, k) {
        ...
        var o = this._get(m, "onAfterSelect");
        if (l) {
        ...
        if (o) {
          o.apply((m.input ? m.input[0] : null), [k, m])
        }
      },
      ...
      _generateHTML: function(ad) {
        ...
        var an = this._get(ad, "showButtonPanel");
        ...
        var ao = this._get(ad, "otherButton");
        var u = (typeof (ao) == "undefined") ? "" : '<button type="button" id="' + ao.id + '" class="ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all" data-handler="' + (ao.clickHide == true ? "hide" : "") + '" data-event="click">' + ao.text + "</button>";
        var aj = (an) ? '<div class="ui-datepicker-buttonpane ui-widget-content">' + (ak ? T : "") + (this._isInRange(ad, ac) ? '<button type="button" class="ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all" data-handler="today" data-event="click">' + ag + "</button>" : "") + u + (ak ? "" : T) + "</div>" : "";
        var N = parseInt(this._get(ad, "firstDay"), 10);
        N = (isNaN(N) ? 0 : N);
        var ap = this._get(ad, "showWeek");
        ...
        for (var H = 0; H < C[0]; H++) {
          ...
          for (var w = 0; w < C[1]; w++) {
            ...
            var U = (ap ? '<th class="ui-datepicker-week-col">' + this._get(ad, "weekHeader") + "</th>" : "");
            for (var aq = 0; aq < 7; aq++) {
              var R = (aq + N) % 7;
              U += "<th" + ((aq + N + 6) % 7 >= 5 ? ' class="ui-datepicker-week-end"' : "") + '><span title="' + x[R] + '">' + A[R] + "</span></th>"
            }
            S += U + "</tr></thead><tbody>";
            ...
            for (var o = 0; o < ab; o++) {
              S += "<tr>";
              var ah = (!ap ? "" : '<td class="ui-datepicker-week-col">' + this._get(ad, "calculateWeek")(ae) + "</td>");
              for (var aq = 0; aq < 7; aq++) {
                var am = (af ? af.apply((ad.input ? ad.input[0] : null), [ae]) : [true, ""]);
                var Q = (ae.getMonth() != k);
                var B = (Q && !I) || !am[0] || (t && ae < t) || (D && ae > D);
                var al = ae.getFullYear().toString() + ae.getMonth().toString() + ae.getDate().toString();
                ah += '<td class="' + ((aq + N + 6) % 7 >= 5 ? " ui-datepicker-week-end" : "") + (Q ? " ui-datepicker-other-month" : "") + ((ae.getTime() == q.getTime() && k == ad.selectedMonth && ad._keyEvent) || (ai.getTime() == ae.getTime() && ai.getTime() == q.getTime()) ? " " + this._dayOverClass : "") + (B ? " " + this._unselectableClass + " ui-state-disabled" : "") + (Q && !v ? "" : " " + am[1] + (ae.getTime() == n.getTime() ? " " + this._currentClass : "") + (ae.getTime() == G.getTime() ? " ui-datepicker-today" : "")) + '"' + ((!Q || v) && am[2] ? ' title="' + am[2] + '"' : "") + (B ? "" : ' data-handler="selectDay" data-event="click" data-month="' + ae.getMonth() + '" data-year="' + ae.getFullYear() + '"') + ">" + (Q && !v ? "&#xa0;" : (B ? '<span class="ui-state-default">' + ae.getDate() + "</span>" : "<a data-date=fk-" + al + ' class="ui-state-default' + (ae.getTime() == G.getTime() ? " ui-state-highlight" : "") + (ae.getTime() == n.getTime() ? " ui-state-active" : "") + (Q ? " ui-priority-secondary" : "") + '" href="#">' + ae.getDate() + "</a>")) + "</td>";
      ...
      _generateMonthYearHeader: function(o, m, w, q, u, x, s, k) {
        ...
        var E = this._get(o, "changeYear");
        var F = this._get(o, "showMonthAfterYear");
        var t = '<div class="ui-datepicker-title">';
        var B = "";
        if (x || !n) {
          B += '<span class="ui-datepicker-month">' + s[m] + "</span>"
        } else {
          var C = (q && q.getFullYear() == w);
          var l = (u && u.getFullYear() == w);
          B += '<select class="ui-datepicker-month" data-handler="selectMonth" data-event="change">';
          for (var D = 0; D < 12; D++) {
            if ((!C || D >= q.getMonth()) && (!l || D <= u.getMonth())) {
              B += '<option value="' + D + '"' + (D == m ? ' selected="selected"' : "") + ">" + k[D] + "</option>"
            }
          }
          B += "</select>"
        }
        ...
        if (!o.yearshtml) {
          ...
          if (x || !E) {
            ...
            var A = this._get(o, "yearSuffix");
            var y = function(H) {
              var G = (H.match(/c[+-].*/) ? w + parseInt(H.substring(1), 10) : (H.match(/[+-].*/) ? p + parseInt(H, 10) : parseInt(H, 10)));
              return (isNaN(G) ? p : G)
            };
            var v = y(z[0]);
            ...
            for (; v <= r; v++) {
              o.yearshtml += '<option value="' + v + '"' + (v == w ? ' selected="selected"' : "") + ">" + v + A + "</option>"
            }
            o.yearshtml += "</select>";
            t += o.yearshtml;
            o.yearshtml = null
...
jQuery(function(d) {
  ...
  d.datepicker.setDefaults(d.datepicker.regional["zh-CN"])
});
(function(b, c) {
...
(function(e, g) {
    ...
    e.widget("ui.tabs", {
      ...
      _showRectangularBlock: function(o, l, n) {
        var j = o.find("." + n + "");
        var h = o.find("li");
        var k = 0;
        if (!j.length) {
          o.append("<div class='" + n + "'></div>")
        }
        for (var m = 0; m < l; m++) {
          if (h[m].style.display == "none") {
            continue
          }
          k += e(h[m]).height() + Math.abs(parseInt(e(h[m]).css("margin-top").replace("px", "")))
        }
        if (!j.length) {
          j = o.find("." + n + "");
          j.css("top", k)
      ...
      _extendedLinePosition: function(q, n) {
        var h, k, j, m, s, t, p, l, r, u;
        h = this.options;
        if (h.popupID == 0) {
          return
        }
        u = h.reduceELHeight;
        j = Fai.top.$("#popupWindow" + h.popupID);
        s = j.height();
        m = j.find(".pWHead").height();
        t = j.find(".pWBottom");
        p = t.height();
        l = j.find(".pWCenter .tabs_extendedLine");
        r = 0;
        if (l.length == 0 || this.lis.length == 0) {
          return
        }
        for (var k = 0; k < n + 1; k++) {
          if (q[k].style.display == "none") {
            continue
          }
          r += e(q[k]).height() + Math.abs(parseInt(e(q[k]).css("margin-top").replace("px", "")))
        }
        l.show().height(s - m - r - p - u).css("top", m + r);
        t.css("border-top", "1px solid #e3e2e8")
      },
      _tabify: function(v) {
        var w = this
          , k = this.options
          , p = k.version == 2 ? true : false
          , j = /^#.+/;
        ...
        this.anchors.each(function(y, o) {
          var x = e(o).attr("href");
          var z = x.split("#")[0], A;
          if (z && (z === location.toString().split("#")[0] || (A = e("base")[0]) && z === A.href)) {
            x = o.hash;
            o.href = x
          }
          if (j.test(x)) {
            w.panels = w.panels.add(w.element.find(w._sanitizeSelector(x)))
          } else {
            if (x && x !== "#") {
              ...
              if (!B.length) {
                if (p) {
                  B = e(k.panelTemplate).attr("id", C).addClass("ui-tabs-panel-v2 ui-widget-content-v2 ui-corner-bottom-v2").insertAfter(w.panels[y - 1] || w.list)
                } else {
                  B = e(k.panelTemplate).attr("id", C).addClass("ui-tabs-panel ui-widget-content ui-corner-bottom").insertAfter(w.panels[y - 1] || w.list)
                }
        ...
        if (v) {
          if (p) {
            this.element.addClass("ui-tabs-v2 ui-widget-v2 ui-widget-content-v2 ui-corner-all-v2");
            this.list.addClass("ui-tabs-nav-v2 ui-helper-reset-v2 ui-helper-clearfix-v2 ui-widget-header-v2 ui-corner-all-v2");
            this.lis.addClass("ui-state-default-v2 ui-corner-top-v2");
            this.panels.addClass("ui-tabs-panel-v2 ui-widget-content-v2 ui-corner-bottom-v2")
          } else {
          ...
          k.disabled = e.unique(k.disabled.concat(e.map(this.lis.filter(".ui-state-disabled"), function(x, o) {
          ...
          if (k.selected >= 0 && this.anchors.length) {
            w.element.find(w._sanitizeSelector(w.anchors[k.selected].hash)).removeClass("ui-tabs-hide");
            this.lis.eq(k.selected).addClass("ui-tabs-selected ui-state-active");
            if (p) {
              w._showRectangularBlock(this.list, k.selected, "rectangularBlock");
              w._extendedLinePosition(this.lis, k.selected);
              setTimeout(function() {
                w.lis.addClass("ui-state-animation-v2")
              }, 0)
            }
            w.element.queue("tabs", function() {
              w._trigger("show", null, w._ui(w.anchors[k.selected], w.element.find(w._sanitizeSelector(w.anchors[k.selected].hash))[0]))
            });
          ...
          e(window).bind("unload", function() {
            ...
            if (k.popupID && p) {
              Fai.top.$("#popupWindow" + k.popupID).find(".pWCenter .tabs_extendedLine").hide()
            }
          })
        } else {
          k.selected = this.lis.index(this.lis.filter(".ui-tabs-selected"));
        ...
        for (var n = 0, u; (u = this.lis[n]); n++) {
          e(u)[e.inArray(n, k.disabled) != -1 && !e(u).hasClass("ui-tabs-selected") ? "addClass" : "removeClass"]("ui-state-disabled")
        }
        if (k.cache === false) {
        ...
        var s = q ? function(i, o) {
            e(i).closest("li").addClass("ui-tabs-selected ui-state-active");
            if (p) {
              w._showRectangularBlock(e(i).closest("li").parent(), e(i).closest("li").index(), "rectangularBlock");
              w._extendedLinePosition(e(i).closest("li").parent().find("li"), e(i).closest("li").index())
            }
            o.hide().removeClass("ui-tabs-hide").animate(q, q.duration || "normal", function() {
              l(o, q);
              w._trigger("show", null, w._ui(i, o[0]))
            })
          }
          : function(i, o) {
            e(i).closest("li").addClass("ui-tabs-selected ui-state-active");
        ...
        this.anchors.bind(k.event + ".tabs", function() {
          var o = this
            , y = e(o).closest("li")
            , i = w.panels.filter(":not(.ui-tabs-hide)")
            , x = w.element.find(w._sanitizeSelector(o.hash));
          if ((y.hasClass("ui-tabs-selected") && !k.collapsible) || y.hasClass("ui-state-disabled") || y.hasClass("ui-state-processing") || w.panels.filter(":animated").length || w._trigger("select", null, w._ui(this, x[0])) === false) {
            this.blur();
            return false
          }
          ...
          w.abort();
          if (k.collapsible) {
            if (y.hasClass("ui-tabs-selected")) {
              k.selected = -1;
              if (k.cookie) {
      ...
      destroy: function() {
        var i = this.options;
        var h = i.version == 2 ? true : false;
        this.abort();
        this.element.unbind(".tabs").removeData("tabs");
        if (h) {
          this.element.removeClass("ui-tabs-v2 ui-widget-v2 ui-widget-content-v2 ui-corner-all-v2 ui-tabs-collapsible");
          this.list.removeClass("ui-tabs-nav-v2 ui-helper-reset-v2 ui-helper-clearfix-v2 ui-widget-header-v2 ui-corner-all-v2")
        } else {
        ...
        this.lis.unbind(".tabs").add(this.panels).each(function() {
          if (e.data(this, "destroy.tabs")) {
            ...
            if (h) {
              e(this).removeClass(["ui-state-animation-v2", "ui-state-default-v2", "ui-corner-top-v2", "ui-tabs-selected", "ui-state-active", "ui-state-hover", "ui-state-focus", "ui-state-disabled", "ui-tabs-panel-v2", "ui-widget-content-v2", "ui-corner-bottom-v2", "ui-tabs-hide"].join(" "))
            } else {
      ...
      add: function(h, n, l) {
        ...
        var p = this
          , j = this.options
          , k = j.version == 2 ? true : false
          , m = e(j.tabTemplate.replace(/#\{href\}/g, h).replace(/#\{label\}/g, n))
          , i = !h.indexOf("#") ? h.replace("#", "") : this._tabId(e("a", m)[0]);
        if (k) {
          m.addClass("ui-state-default-v2 ui-corner-top-v2").data("destroy.tabs", true)
        } else {
          m.addClass("ui-state-default ui-corner-top").data("destroy.tabs", true)
        }
        var q = p.element.find("#" + i);
        if (!q.length) {
          q = e(j.panelTemplate).attr("id", i).data("destroy.tabs", true)
        }
        if (k) {
          q.addClass("ui-tabs-panel-v2 ui-widget-content-v2 ui-corner-bottom-v2 ui-tabs-hide")
        } else {
        ...
        j.disabled = e.map(j.disabled, function(r, o) {
          return r >= l ? ++r : r
        });
        this._tabify();
...
(function(c) {
    jQuery.fn.extend({
      imageSwitch: function(e) {
        ...
        return c(this).each(function() {
          var B = c(this);
          var F = d.width || B.width();
          var h = d.height || B.height();
          var K = d.data.length;
          var f = d.index;
          var G = F < B.width() ? F : B.width();
          B.css("overflow", "hidden");
          B.height(h);
          if (d.width && d.from == "module") {
            B.width(d.width)
          }
          var t = c('<div class="imageSwitchShowName photoSwitchBg" />').css({
            position: "absolute",
            display: "none",
            zIndex: 2,
            height: 30 + "px",
            "line-height": 30 + "px",
            width: c.browser.msie && c.browser.version == 6 ? c(B).parent().parent().width() + "px" : c(B).parent().width() + "px"
          }).appendTo(B);
          if (K == 0) {
          ...
          if (d.btnType != 0) {
            var D = c('<div class="imageSwitchBtnArea"/>').css({
              position: "absolute",
              zIndex: 3,
              display: w
            }).appendTo(B)
          ...
          c("<div />").appendTo(B);
          var i = c('<div class="switchGroup"/>').css({
            width: (d.width <= B.width()) ? d.width : "100%",
            position: "relative",
            overflow: "hidden"
          }).height("y,show-y".indexOf(d.animateStyle) != -1 ? h * K : h).appendTo(B);
          var v = 0;
          var r = "100%";
          if (d.btnType == 0) {
            d.animateStyle = "x"
          }
          c.each(d.data, function(P, L) {
            var U = "onclick='";
            if (L.click != null) {
              U += L.click
            }
            ...
            U += "'";
            var M = c('<a hidefocus="true" style="outline:none;" ' + U + "/>").css({
              width: r,
              height: h + "px",
              ...
              position: S,
              display: "block",
              "float": "x,show-x".indexOf(d.animateStyle) != -1 ? "left" : ""
            }).attr({
              id: "photoList" + (L.pic || P),
              href: L.href ? L.href : "javascript:;",
            ...
            if (L.width && L.width > 0) {
              M.html("<img src='" + b(L.src, T, O) + "' width='" + L.width + "' height='" + L.height + "' alt='" + L.name + "'>")
            } else {
              M.css("background-image", "url(" + L.src + ")")
            }
            if (d.btnType == 1) {
              var N = c('<a class="imageSwitchBtn" />').appendTo(D).html("<span>" + (P + 1) + "</span>")
            } else {
              if (d.btnType == 2) {
                var N = c('<a class="imageSwitchBtn_dot" />').appendTo(D)
              }
            }
            var Q = c('<span class="spanHiddenName"/>').appendTo(t).css("margin-left", 10 + "px").html(Fai.encodeHtml(L.name));
            if (d.btnType != 0) {
              if (P == f) {
                if (d.btnType == 1) {
                  N.addClass("imageSwitchBtnSel")
                } else {
                  N.addClass("imageSwitchBtnSel_dot")
                }
                Q.addClass("spanShowName")
              }
            }
            if (d.btnType == 1) {
              v += c(N).outerWidth(true)
            }
          });
          if (d.btnType == 0) {
            if (K > 1) {
              var E = c('<a class="imageSwitchBtn_arrow arrow_prev" />').appendTo(B).css("position", "absolute").css("zIndex", 3);
              var j = c('<a class="imageSwitchBtn_arrow arrow_next" />').appendTo(B).css("position", "absolute").css("zIndex", 3)
            }
          } else {}
          if (d.btnType == 1) {
            D.width(v)
              ...
              if (Fai.top.$("#banner").length > 0) {
                if (d.btnType == 1) {
                  if (F >= B.width()) {
                    ...
                    D.css("right", "50%");
                    D.css("marginRight", "-" + F / 2 + "px")
                  }
                } else {
                  if (d.btnType == 2) {
                    D.css("left", "50%");
                    D.css("marginLeft", "-" + D.width() / 2 + "px")
                  }
                }
              } else {
                D.css("left", (u - v) + "px")
          ...
          var y = t.children("span");
          var A = u - v;
          if (K > 1) {
            if (d.btnType != 0) {
              t.css("top", (g - D.height() - 9) + "px").css("left", 0 + "px")
            } else {
              t.css("top", g + "px").css("left", 0 + "px")
            }
            var k = c.browser.msie && c.browser.version == 6 ? parseInt(n() + 30) : parseInt(n() + 20);
            if (A > k) {
              if (d.btnType == 1) {
                D.css("top", (g - D.height() - 6) + "px")
              } else {
                if (d.btnType == 2) {
                  D.css("bottom", "5px")
                } else {
                  E.css("top", "50%");
                  E.css("marginTop", "-" + E.height() / 2 + "px");
                  j.css("top", "50%");
                  j.css("marginTop", "-" + j.height() / 2 + "px")
                }
              }
            } else {
              if (d.btnType == 1) {
                D.css("top", (g - D.height() - 30) + "px")
              } else {
                if (d.btnType == 2) {
                  D.css("bottom", "5px")
                } else {
          ...
          if ("o,show,none".indexOf(d.animateStyle) != -1) {
            H.each(function(L, M) {
              if (f != L) {
                c(this).hide()
              }
              c(this).css("position", "absolute");
              c(this).css("left", "0");
              c(this).css("top", "0")
            })
          }
          ...
          if (K > 1) {
            if (d.btnType != 0) {
              s.click(function(M) {
                ...
                if (d.btnType == 1) {
                  s.eq(f).removeClass("imageSwitchBtnSel");
                  s.eq(L).addClass("imageSwitchBtnSel")
                } else {
                  s.eq(f).removeClass("imageSwitchBtnSel_dot");
                  s.eq(L).addClass("imageSwitchBtnSel_dot")
                }
                y.eq(f).removeClass("spanShowName");
                y.eq(L).addClass("spanShowName");
                switch (d.animateStyle) {
                  case "o":
                    H.eq(f).fadeOut(d.animateTime, "failinear");
                    H.eq(L).fadeIn(d.animateTime, "failinear");
                    break;
                  case "x":
                    i.animate({
                      marginLeft: -L * F
                ...
                if (N) {
                  N.css({
                    width: d.data[L].imgWidth,
                    left: (B.width() - d.data[L].imgWidth) / 2 + "px"
                  });
                  N.css({
                    height: d.data[L].imgHeight,
                    top: (B.height() - d.data[L].imgHeight) / 2 + "px"
                  })
                }
              });
              function x() {
              ...
              function C(L) {
                ...
                Fai.stopInterval(J);
                y.eq(o).removeClass("spanShowName");
                if (o == K - 1) {
                  o = -1
                }
                y.eq(++o).addClass("spanShowName");
                Fai.top.$("#arrowImg" + o).css({
                  left: r
                });
                ...
                if (M) {
                  M.css({
                    width: d.data[o].imgWidth,
                    left: (B.width() - d.data[o].imgWidth) / 2 + "px"
                  });
                  M.css({
                    height: d.data[o].imgHeight,
                    top: (B.height() - d.data[o].imgHeight) / 2 + "px"
                  })
                }
                setTimeout(function() {
                  Fai.startInterval(J)
              ...
              m.on("click", C);
              m.hover(function() {
                m.addClass("arrow_next_hover")
              }, function() {
                m.removeClass("arrow_next_hover")
              });
              function z(L) {
                L.stopPropagation();
                if (Fai.top.$(".arrowImg").is(":animated")) {
              ...
              l.on("click", z);
              l.hover(function() {
                l.addClass("arrow_prev_hover")
              }, function() {
                l.removeClass("arrow_prev_hover")
              });
              function I() {
                m.click()
              }
            }
            if (d.manuallyCarousel) {
              Fai.stopInterval(J)
            } else {
              var J = "imageSwitch" + Math.random();
              B.data("intervalId", J);
              if (d.btnType != 0) {
                Fai.addInterval(J, x, d.playTime)
              } else {
                Fai.addInterval(J, I, d.playTime)
              }
              Fai.startInterval(J);
              if (typeof d.mouseoverId != "undefined") {
                c(Fai.top.$(B).find("." + d.mouseoverId)[0]).mouseover(function() {
                  Fai.stopInterval(J)
                });
                c(Fai.top.$(B).find("." + d.mouseoverId)[0]).mouseout(function() {
                  Fai.startInterval(J)
                })
              } else {
    ...
    function b(j, d, i) {
      if (d == null || i == null || j == null || d.width <= 0 || d.height <= 0 || i.width <= 0 || i.height <= 0 || j == "") {
        return j
      }
      if (j.indexOf("no-pic") > -1) {
        return j
      }
      if (j.indexOf("image/materialLib") > -1) {
        return j
      }
      j = j.replace(/!\d+x\d+/, "");
      var k = 0;
      if (d.width > i.width) {
        var h = d.width / i.width;
        i.width *= h;
        i.height *= h
      }
      if (d.height > i.height) {
        var h = d.height / i.height;
        i.width *= h;
        i.height *= h
      }
      if (i.width >= i.height) {
      ...
      if (k == 0) {
        ...
        var e = j.substring(g + 1, f);
        return j.substring(0, f) + "!" + k + "x" + k + j.substring(f)
      }
    }
  }
)(jQuery);
(function(c) {
    ...
    function b(n, t) {
      ...
      ;
      this.controlLoop = function(H) {
        if (B) {
          var E = (u - q.offset().left - (i * 0.5)) >> 0;
          var I = (s - q.offset().top - (r * 0.5)) >> 0;
          if (E < 0) {
            E = 0
          } else {
          ...
          if (I < 0) {
            ...
            if (I > (q.parent().outerHeight() - r)) {
              I = (q.parent().outerHeight() - r)
            }
          }
          B.css({
            left: E,
          ...
          B.css("background-position", (-E) + "px " + (-I) + "px");
          g = (((E) / q.parent().outerWidth()) * h.width) >> 0;
          f = (((I) / q.parent().outerHeight()) * h.height) >> 0;
          x += (g - x) / t.smoothMove;
          v += (f - v) / t.smoothMove;
          if (t.imageWidth < t.maxWidth) {
            var G = (q.parent().outerWidth() / 2) - (h.width / 2);
            x = -G
          } else {
            if (t.imageHeight < t.maxHeight) {
              var F = (q.parent().outerHeight() / 2) - (h.height / 2);
              v = -F
            } else {}
          }
      ...
      this.init = function() {
        ...
        j.bind("mouseenter", this, function(G) {
          ...
          o = O.append(d('<div id="cloud-zoom-big" class="cloud-zoom-big" style="display:none;position:absolute;left:%0px;top:%1px;width:%2px;height:%3px;background-image:url(\'%4\');background-repeat:no-repeat;z-index:3;"></div>', L, H, N, M, h.src)).find(":last");
...
(function(b) {
    ...
    b.datepicker._gotoToday = function(j) {
      var g = this._getInst(b(j)[0])
        , i = g.dpDiv;
      this._base_gotoToday(j);
      var f = this._get(g, "timepicker");
      c(f);
      var e = new Date();
      var d = d = new Date(e.getFullYear(),e.getMonth(),e.getDate());
      var h = this._get(g, "hourAccuracy") ? d : e;
      this._setTime(g, h);
      b(".ui-datepicker-today", i).click()
    }
    ;
    var c = function(g, f) {
...
(function(g, l, n, e) {
  var k = "stellar"
    ...
    , b = (function() {
    var s = /^(Moz|Webkit|Khtml|O|ms|Icab)(?=[A-Z])/, q = g("script")[0].style, r = "", t;
    for (t in q) {
      if (s.test(t)) {
        r = t.match(s)[0];
        break
  ...
  p.prototype = {
    ...
    _findParticles: function() {
      ...
      this.$element.find("[data-stellar-ratio]").each(function(B) {
        var F = g(this), D, I, v, E, A, u, H, z, x, C = 0, G = 0, w = 0, y = 0;
    ...
    _findBackgrounds: function() {
      var q = this, t = this._getScrollLeft(), s = this._getScrollTop(), r;
      this.backgrounds = [];
      ...
      r.each(function() {
        var F = g(this), v = i(F), D, I, w, E, B, u, A, y, H, C = 0, G = 0, x = 0, z = 0;
    ...
    _repositionElements: function() {
      var w = this._getScrollLeft(), t = this._getScrollTop(), C, G, B, F, r, E, x, v = true, q = true, A, D, u, s, z;
      ...
      for (z = this.particles.length - 1; z >= 0; z--) {
        B = this.particles[z];
        F = (B.isFixed ? 1 : 0);
        if (this.options.horizontalScrolling) {
          A = (w + B.horizontalOffset + this.viewportOffsetLeft + B.startingPositionLeft - B.startingOffsetLeft + B.parentOffsetLeft) * -(B.stellarRatio + F - 1) + B.startingPositionLeft;
          u = A - B.startingPositionLeft + B.startingOffsetLeft
        } else {
          A = B.startingPositionLeft;
          u = B.startingOffsetLeft
        }
        if (this.options.verticalScrolling) {
          D = (t + B.verticalOffset + this.viewportOffsetTop + B.startingPositionTop - B.startingOffsetTop + B.parentOffsetTop) * -(B.stellarRatio + F - 1) + B.startingPositionTop;
          s = D - B.startingPositionTop + B.startingOffsetTop
        } else {
          D = B.startingPositionTop;
          s = B.startingOffsetTop
        }
        if (this.options.hideDistantElements) {
          q = !this.options.horizontalScrolling || u + B.width > (B.isFixed ? 0 : w) && u < (B.isFixed ? 0 : w) + this.viewportWidth + this.viewportOffsetLeft;
          v = !this.options.verticalScrolling || s + B.height > (B.isFixed ? 0 : t) && s < (B.isFixed ? 0 : t) + this.viewportHeight + this.viewportOffsetTop
        }
        if (q && v) {
          if (B.isHidden) {
          ...
          this._setPosition(B.$element, A, B.startingPositionLeft, D, B.startingPositionTop)
        } else {
          if (!B.isHidden) {
            this.options.hideElement(B.$element);
            B.isHidden = true
          }
        }
      }
      var y = Fai.top.$("#g_main").offset().top;
      for (z = this.backgrounds.length - 1; z >= 0; z--) {
        r = this.backgrounds[z];
        F = (r.isFixed ? 0 : 1);
        E = (this.options.horizontalScrolling ? (w + r.horizontalOffset - this.viewportOffsetLeft - r.startingOffsetLeft + r.parentOffsetLeft - r.startingBackgroundPositionLeft) * (F - r.stellarRatio) + "px" : r.startingValueLeft);
        x = (this.options.verticalScrolling ? (t + r.verticalOffset - (this.viewportOffsetTop - y) - (r.startingOffsetTop - y) + r.parentOffsetTop - r.startingBackgroundPositionTop) * (F - r.stellarRatio) + y + "px" : r.startingValueTop);
        c(r.$element, E, x)
      }
    },
...
jQuery.effects || (function(i, f) {
    ...
    function n(q, o) {
      ...
      do {
        ...
        o = "backgroundColor"
      } while (q = q.parentNode);return l(p)
    }
    var b = {
      aqua: [0, 255, 255],
      azure: [240, 255, 255],
    ...
    function h() {
      var r = document.defaultView ? document.defaultView.getComputedStyle(this, null) : this.currentStyle, s = {}, p, q;
    ...
    i.effects.animateClass = function(o, p, r, q) {
      ...
      return this.queue(function() {
        var v = i(this), s = v.attr("style") || " ", w = c(h.call(this)), u, t = v.attr("class") || "";
...
(function(g) {
    g.ui.timepicker = g.ui.timepicker || {};
    if (g.ui.timepicker.version) {
      return
    }
    g.extend(g.ui, {
      timepicker: {
    ...
    g.extend(c.prototype, {
      ...
      _newInst: function(r, j) {
        var p = new c(), n = {}, u = {}, s, l;
        for (var q in this._defaults) {
        ...
        s = {
          beforeShow: function(o, i) {
            if (g.isFunction(p._defaults.evnts.beforeShow)) {
              return p._defaults.evnts.beforeShow.call(r[0], o, i, p)
            }
          },
          onChangeMonthYear: function(o, v, i) {
            if (!i.settings.autoChangeMonthYear) {
              return
            }
            p._updateDateTime(i);
            if (g.isFunction(p._defaults.evnts.onChangeMonthYear)) {
              p._defaults.evnts.onChangeMonthYear.call(r[0], o, v, i, p)
            }
          },
          onClose: function(o, i) {
            if (p.timeDefined === true && r.val() !== "") {
              p._updateDateTime(i)
            }
            if (g.isFunction(p._defaults.evnts.onClose)) {
              p._defaults.evnts.onClose.call(r[0], o, i, p)
            }
          }
        };
        for (l in s) {
        ...
        p._defaults = g.extend({}, this._defaults, n, j, s, {
          evnts: u,
          timepicker: p
        });
        p.amNames = g.map(p._defaults.amNames, function(i) {
          return i.toUpperCase()
        });
        p.pmNames = g.map(p._defaults.pmNames, function(i) {
          return i.toUpperCase()
        });
        if (typeof (p._defaults.controlType) === "string") {
          if (p._defaults.controlType == "slider" && g.fn.slider === undefined) {
            p._defaults.controlType = "select"
          }
          p.control = p._controls[p._defaults.controlType]
        } else {
          p.control = p._defaults.controlType
        }
        if (p._defaults.timezoneList === null) {
          var t = ["-1200", "-1100", "-1000", "-0930", "-0900", "-0800", "-0700", "-0600", "-0500", "-0430", "-0400", "-0330", "-0300", "-0200", "-0100", "+0000", "+0100", "+0200", "+0300", "+0330", "+0400", "+0430", "+0500", "+0530", "+0545", "+0600", "+0630", "+0700", "+0800", "+0845", "+0900", "+0930", "+1000", "+1030", "+1100", "+1130", "+1200", "+1245", "+1300", "+1400"];
          if (p._defaults.timezoneIso8601) {
            t = g.map(t, function(i) {
              return i == "+0000" ? "Z" : (i.substring(0, 3) + ":" + i.substring(3))
            })
          }
          p._defaults.timezoneList = t
        }
        p.timezone = p._defaults.timezone;
        p.hour = p._defaults.hour;
        p.minute = p._defaults.minute;
        p.second = p._defaults.second;
        p.millisec = p._defaults.millisec;
        p.ampm = "";
        p.$input = r;
        if (j.altField) {
          p.$altInput = g(j.altField).css({
            cursor: "pointer"
          }).focus(function() {
            r.trigger("focus")
          })
        }
        if (p._defaults.minDate === 0 || p._defaults.minDateTime === 0) {
          p._defaults.minDate = new Date()
        }
        if (p._defaults.maxDate === 0 || p._defaults.maxDateTime === 0) {
          p._defaults.maxDate = new Date()
        }
        if (p._defaults.minDate !== undefined && p._defaults.minDate instanceof Date) {
          p._defaults.minDateTime = new Date(p._defaults.minDate.getTime())
        }
        if (p._defaults.minDateTime !== undefined && p._defaults.minDateTime instanceof Date) {
          p._defaults.minDate = new Date(p._defaults.minDateTime.getTime())
        }
        if (p._defaults.maxDate !== undefined && p._defaults.maxDate instanceof Date) {
          p._defaults.maxDateTime = new Date(p._defaults.maxDate.getTime())
        }
        if (p._defaults.maxDateTime !== undefined && p._defaults.maxDateTime instanceof Date) {
          p._defaults.maxDate = new Date(p._defaults.maxDateTime.getTime())
        }
        p.$input.bind("focus", function() {
          p._onFocus()
        });
        return p
      },
      _addTimePicker: function(j) {
        var i = (this.$altInput && this._defaults.altFieldTimeOnly) ? this.$input.val() + " " + this.$altInput.val() : this.$input.val();
        this.timeDefined = this._parseTime(i);
        this._limitMinMaxDateTime(j, false);
        this._injectTimePicker()
      },
      _parseTime: function(k, j) {
        if (!this.inst) {
          this.inst = g.datepicker._getInst(this.$input[0])
        }
        if (j || !this._defaults.timeOnly) {
          var n = g.datepicker._get(this.inst, "dateFormat");
          try {
            var i = d(n, this._defaults.timeFormat, k, g.datepicker._getFormatConfig(this.inst), this._defaults);
            if (!i.timeObj) {
              return false
            }
            g.extend(this, i.timeObj)
          } catch (l) {
            return false
          }
          return true
        } else {
          var m = g.datepicker.parseTime(this._defaults.timeFormat, k, this._defaults);
          if (!m) {
            return false
          }
          g.extend(this, m);
      ...
      _injectTimePicker: function() {
        var w = this.inst.dpDiv
          , z = this.inst.settings
          , k = this
          , u = ""
          , n = ""
          , E = {}
          , r = {}
          , x = null;
        if (w.find("div.ui-timepicker-div").length === 0 && z.showTimepicker) {
          var p = ' style="display:none;"'
            , s = '<div class="ui-timepicker-div' + (z.isRTL ? " ui-timepicker-rtl" : "") + '"><dl><dt class="ui_tpicker_time_label"' + ((z.showTime) ? "" : p) + ">" + z.timeText + '</dt><dd class="ui_tpicker_time"' + ((z.showTime) ? "" : p) + "></dd>";
          for (var F = 0, B = this.units.length; F < B; F++) {
            u = this.units[F];
            n = u.substr(0, 1).toUpperCase() + u.substr(1);
            E[u] = parseInt((z[u + "Max"] - ((z[u + "Max"] - z[u + "Min"]) % z["step" + n])), 10);
            r[u] = 0;
            s += '<dt class="ui_tpicker_' + u + '_label"' + ((z["show" + n]) ? "" : p) + ">" + z[u + "Text"] + '</dt><dd class="ui_tpicker_' + u + '"><div class="ui_tpicker_' + u + '_slider"' + ((z["show" + n]) ? "" : p) + "></div>";
            if (z["show" + n] && z[u + "Grid"] > 0) {
              s += '<div style="padding-left: 1px"><table class="ui-tpicker-grid-label"><tr>';
              if (u == "hour") {
                for (var G = z[u + "Min"]; G <= E[u]; G += parseInt(z[u + "Grid"], 10)) {
                  r[u]++;
                  var t = (z.ampm && G > 12) ? G - 12 : G;
                  if (t < 10) {
                    t = "0" + t
                  }
                  if (z.ampm) {
                  ...
                  s += '<td data-for="' + u + '">' + t + "</td>"
                }
              } else {
                for (var A = z[u + "Min"]; A <= E[u]; A += parseInt(z[u + "Grid"], 10)) {
                  r[u]++;
                  s += '<td data-for="' + u + '">' + ((A < 10) ? "0" : "") + A + "</td>"
                }
              }
              s += "</tr></table></div>"
            }
          ...
          s += '<dt class="ui_tpicker_timezone_label"' + ((z.showTimezone) ? "" : p) + ">" + z.timezoneText + "</dt>";
          s += '<dd class="ui_tpicker_timezone" ' + ((z.showTimezone) ? "" : p) + "></dd>";
          s += "</dl></div>";
          var j = g(s);
          if (z.timeOnly === true) {
            j.prepend('<div class="ui-widget-header ui-helper-clearfix ui-corner-all"><div class="ui-datepicker-title">' + z.timeOnlyTitle + "</div></div>");
            w.find(".ui-datepicker-header, .ui-datepicker-calendar").hide()
          }
          for (var F = 0, B = k.units.length; F < B; F++) {
            u = k.units[F];
            n = u.substr(0, 1).toUpperCase() + u.substr(1);
            k[u + "_slider"] = k.control.create(k, j.find(".ui_tpicker_" + u + "_slider"), u, k[u], z[u + "Min"], E[u], z["step" + n]);
            if (z["show" + n] && z[u + "Grid"] > 0) {
              x = 100 * r[u] * z[u + "Grid"] / (E[u] - z[u + "Min"]);
              j.find(".ui_tpicker_" + u + " table").css({
                width: x + "%",
                marginLeft: z.isRTL ? "0" : ((x / (-2 * r[u])) + "%"),
                marginRight: z.isRTL ? ((x / (-2 * r[u])) + "%") : "0",
                borderCollapse: "collapse"
              }).find("td").click(function(I) {
                var J = g(this)
                  , l = J.html()
                  , o = J.data("for");
                if (o == "hour" && z.ampm) {
                  var i = l.substring(2).toLowerCase()
                    , m = parseInt(l.substring(0, 2), 10);
                  if (i == "a") {
                    if (m == 12) {
                      l = 0
                    } else {
                ...
                k.control.value(k, k[o + "_slider"], parseInt(l, 10));
                k._onTimeChange();
                k._onSelectHandler()
              }).css({
                cursor: "pointer",
          ...
          g.fn.append.apply(this.timezone_select, g.map(z.timezoneList, function(l, i) {
            return g("<option />").val(typeof l == "object" ? l.value : l).text(typeof l == "object" ? l.label : l)
          }));
          if (typeof (this.timezone) != "undefined" && this.timezone !== null && this.timezone !== "") {
            var H = new Date(this.inst.selectedYear,this.inst.selectedMonth,this.inst.selectedDay,12);
            var D = g.timepicker.timeZoneOffsetString(H);
            if (D == this.timezone) {
              h(k)
            } else {
            ...
            if (typeof (this.hour) != "undefined" && this.hour !== null && this.hour !== "") {
              this.timezone_select.val(z.defaultTimezone)
            } else {
              h(k)
            }
          }
          this.timezone_select.change(function() {
            k._defaults.useLocalTimezone = false;
            k._onTimeChange()
          });
          var v = w.find(".ui-datepicker-buttonpane");
          if (v.length) {
            v.before(j)
          } else {
            w.append(j)
          ...
          if (this._defaults.addSliderAccess) {
            var C = this._defaults.sliderAccessArgs
              , q = this._defaults.isRTL;
            C.isRTL = q;
            setTimeout(function() {
              if (j.find(".ui-slider-access").length === 0) {
                j.find(".ui-slider:visible").sliderAccess(C);
                var i = j.find(".ui-slider-access:eq(0)").outerWidth(true);
                if (i) {
                  j.find("table:visible").each(function() {
                    var m = g(this)
                      , l = m.outerWidth()
                      , K = m.css(q ? "marginRight" : "marginLeft").toString().replace("%", "")
                      , J = l - i
                      , I = ((K * J) / l) + "%"
                      , o = {
                      width: J,
      ...
      _limitMinMaxDateTime: function(s, t) {
        var j = this._defaults
          , q = new Date(s.selectedYear,s.selectedMonth,s.selectedDay);
        if (!this._defaults.showTimepicker) {
          return
        }
        if (g.datepicker._get(s, "minDateTime") !== null && g.datepicker._get(s, "minDateTime") !== undefined && q) {
          var i = g.datepicker._get(s, "minDateTime")
            , r = new Date(i.getFullYear(),i.getMonth(),i.getDate(),0,0,0,0);
          if (this.hourMinOriginal === null || this.minuteMinOriginal === null || this.secondMinOriginal === null || this.millisecMinOriginal === null) {
            this.hourMinOriginal = j.hourMin;
            this.minuteMinOriginal = j.minuteMin;
            this.secondMinOriginal = j.secondMin;
            this.millisecMinOriginal = j.millisecMin
          }
          if (s.settings.timeOnly || r.getTime() == q.getTime()) {
        ...
        if (g.datepicker._get(s, "maxDateTime") !== null && g.datepicker._get(s, "maxDateTime") !== undefined && q) {
          var u = g.datepicker._get(s, "maxDateTime")
            , l = new Date(u.getFullYear(),u.getMonth(),u.getDate(),0,0,0,0);
          if (this.hourMaxOriginal === null || this.minuteMaxOriginal === null || this.secondMaxOriginal === null) {
            this.hourMaxOriginal = j.hourMax;
            this.minuteMaxOriginal = j.minuteMax;
            this.secondMaxOriginal = j.secondMax;
            this.millisecMaxOriginal = j.millisecMax
          }
          if (s.settings.timeOnly || l.getTime() == q.getTime()) {
            ...
            if (this.hour >= this._defaults.hourMax) {
              ...
              if (this.minute >= this._defaults.minuteMax) {
                ...
                this._defaults.secondMax = u.getSeconds()
              } else {
                if (this.second >= this._defaults.secondMax) {
                  this.second = this._defaults.secondMax;
                  this._defaults.millisecMax = u.getMilliseconds()
                } else {
        ...
        if (t !== undefined && t === true) {
          var m = parseInt((this._defaults.hourMax - ((this._defaults.hourMax - this._defaults.hourMin) % this._defaults.stepHour)), 10)
            , n = parseInt((this._defaults.minuteMax - ((this._defaults.minuteMax - this._defaults.minuteMin) % this._defaults.stepMinute)), 10)
            , p = parseInt((this._defaults.secondMax - ((this._defaults.secondMax - this._defaults.secondMin) % this._defaults.stepSecond)), 10)
            , k = parseInt((this._defaults.millisecMax - ((this._defaults.millisecMax - this._defaults.millisecMin) % this._defaults.stepMillisec)), 10);
          if (this.hour_slider) {
      ...
      _onTimeChange: function() {
        var i = (this.hour_slider) ? this.control.value(this, this.hour_slider) : false
          , q = (this.minute_slider) ? this.control.value(this, this.minute_slider) : false
          , k = (this.second_slider) ? this.control.value(this, this.second_slider) : false
          , m = (this.millisec_slider) ? this.control.value(this, this.millisec_slider) : false
          , n = (this.timezone_select) ? this.timezone_select.val() : false
          , p = this._defaults;
        if (typeof (i) == "object") {
          i = false
        }
        ...
        var j = p[i < 12 ? "amNames" : "pmNames"][0];
        var l = (i != this.hour || q != this.minute || k != this.second || m != this.millisec || (this.ampm.length > 0 && (i < 12) != (g.inArray(this.ampm.toUpperCase(), this.amNames) !== -1)) || ((this.timezone === null && n != this.defaultTimezone) || (this.timezone !== null && n != this.timezone)));
        if (l) {
          if (i !== false) {
            this.hour = i
          }
        ...
        this.formattedTime = g.datepicker.formatTime(this._defaults.timeFormat, this, this._defaults);
        if (this.$timeObj) {
          this.$timeObj.text(this.formattedTime + p.timeSuffix)
        }
      ...
      _onSelectHandler: function() {
        var i = this._defaults.onSelect || this.inst.settings.onSelect;
        var j = this.$input ? this.$input[0] : null;
        if (i && j) {
          i.apply(j, [this.formattedDateTime, this])
        }
      },
      _updateDateTime: function(o) {
        o = this.inst || o;
        var j = g.datepicker._daylightSavingAdjust(new Date(o.selectedYear,o.selectedMonth,o.selectedDay))
          , p = g.datepicker._get(o, "dateFormat")
          , k = g.datepicker._getFormatConfig(o)
          , n = j !== null && this.timeDefined;
        this.formattedDate = g.datepicker.formatDate(p, (j === null ? new Date() : j), k);
        var i = this.formattedDate;
        if (this._defaults.timeOnly === true) {
          i = this.formattedTime
        } else {
          if (this._defaults.timeOnly !== true && (this._defaults.alwaysSetTime || n)) {
            i += this._defaults.separator + this.formattedTime + this._defaults.timeSuffix
          }
        }
        this.formattedDateTime = i;
        if (!this._defaults.showTimepicker) {
          ...
          if (this.$altInput && this._defaults.altFieldTimeOnly === true) {
            ...
            if (this.$altInput) {
              this.$input.val(i);
              var m = ""
                , q = this._defaults.altSeparator ? this._defaults.altSeparator : this._defaults.separator
                , l = this._defaults.altTimeSuffix ? this._defaults.altTimeSuffix : this._defaults.timeSuffix;
              if (this._defaults.altFormat) {
                m = g.datepicker.formatDate(this._defaults.altFormat, (j === null ? new Date() : j), k)
              } else {
              ...
              if (this._defaults.altTimeFormat) {
                m += g.datepicker.formatTime(this._defaults.altTimeFormat, this, this._defaults) + l
              } else {
                m += this.formattedTime + l
              }
              this.$altInput.val(m)
            } else {
              this.$input.val(i)
      ...
      _onFocus: function() {
        if (!this.$input.val() && this._defaults.defaultValue) {
          ...
          var k = g.datepicker._getInst(this.$input.get(0))
            , i = g.datepicker._get(k, "timepicker");
          if (i) {
            if (i._defaults.timeOnly && (k.input.val() != k.lastVal)) {
              try {
                g.datepicker._updateDatepicker(k)
              } catch (j) {
                g.datepicker.log(j)
              }
            }
      ...
      _controls: {
        slider: {
          create: function(j, o, m, p, k, i, l) {
            var n = j._defaults.isRTL;
            return o.prop("slide", null).slider({
              ...
              step: l,
              slide: function(q, r) {
                j.control.value(j, g(this), n ? r.value * -1 : r.value);
                j._onTimeChange()
              },
              stop: function(q, r) {
                j._onSelectHandler()
          ...
          options: function(j, m, l, n) {
            if (j._defaults.isRTL) {
              if (typeof (l) == "string") {
                if (l == "min" || l == "max") {
                  if (n !== undefined) {
                    return m.slider(l, n * -1)
                  }
                  return Math.abs(m.slider(l))
                }
                return m.slider(l)
              }
              var k = l.min
            ...
            if (typeof (l) == "string" && n !== undefined) {
              return m.slider(l, n)
            }
            return m.slider(l)
          },
          value: function(i, j, k) {
            if (i._defaults.isRTL) {
              if (k !== undefined) {
                return j.slider("value", k * -1)
              }
              return Math.abs(j.slider("value"))
            }
            if (k !== undefined) {
              return j.slider("value", k)
            }
            return j.slider("value")
          }
        },
        select: {
          create: function(r, q, u, k, o, t, l) {
            var j = '<select class="ui-timepicker-select" data-unit="' + u + '" data-min="' + o + '" data-max="' + t + '" data-step="' + l + '">'
              , s = r._defaults.timeFormat.indexOf("t") !== -1 ? "toLowerCase" : "toUpperCase"
              , n = 0;
            for (var p = o; p <= t; p += l) {
              j += '<option value="' + p + '"' + (p == k ? " selected" : "") + ">";
              if (u == "hour" && r._defaults.ampm) {
                n = p % 12;
                if (p === 0 || p === 12) {
                  j += "12"
                } else {
                ...
                j += " " + ((p < 12) ? r._defaults.amNames[0] : r._defaults.pmNames[0])[s]()
              } else {
                if (u == "millisec" || p >= 10) {
                  j += p
                } else {
            ...
            j += "</select>";
            q.children("select").remove();
            g(j).appendTo(q).change(function(i) {
              r._onTimeChange();
              r._onSelectHandler()
            });
            return q
          },
          options: function(i, k, j, m) {
            var l = {}
              , n = k.children("select");
            if (typeof (j) == "string") {
              if (m === undefined) {
            ...
            return i.control.create(i, k, n.data("unit"), n.val(), l.min || n.data("min"), l.max || n.data("max"), l.step || n.data("step"))
          },
          value: function(i, j, k) {
            var l = j.children("select");
            if (k !== undefined) {
              return l.val(k)
            }
            return l.val()
        ...
        fkControl: {
          create: function(r, q, u, k, o, t, l) {
            var j = '<div class="fk-timepicker-time" data-unit="' + u + '" data-min="' + o + '" data-max="' + t + '" data-step="' + l + '">', v = r.inst.settings.beforeShowTime, s = r._defaults.timeFormat.indexOf("t") !== -1 ? "toLowerCase" : "toUpperCase", n = 0, w;
            j += "<input type='text' class='fk-timepicker-select' value='" + (k != 0 ? k + "：00" : "00：00") + "' _showTime='0' readOnly>";
            j += "<div class='fk-timeVauleWrap'>";
            if (q.hasClass("ui_tpicker_hour_slider")) {
              for (var p = o; p <= t; p += l) {
                w = typeof v == "function" ? v(p) : true;
                j += '<span class="fk-timeValue ' + (w ? "" : "fk-timeBan") + '" value="' + p + '"' + (p == k ? " selected" : "") + ">";
                if (u == "hour" && r._defaults.ampm) {
                  n = p % 12;
                  if (p === 0 || p === 12) {
                    j += "12：00"
            ...
            j += "</div>";
            j += "</div>";
            q.children(".fk-timepicker-time").remove();
            g(j).appendTo(q);
            q.find(".fk-timepicker-time").off("click.time").on("click.time", function(y) {
              var x = g(this)
                , m = g(this).find(".fk-timeVauleWrap")
                , i = g(this).find(".fk-timepicker-select");
              if (parseInt(i.attr("_showTime")) == 0) {
                m.show();
                i.attr("_showTime", 1).addClass("f-select")
              } else {
                m.hide();
                i.attr("_showTime", 0).removeClass("f-select");
                i
              }
            });
            q.find(".fk-timeVauleWrap").on("click.select", ".fk-timeValue", function(m) {
              var i = g(this).html();
              q.find(".fk-timepicker-select").val(i).attr("_showTime", "0").removeClass(".f-select");
              r._onTimeChange();
              r._onSelectHandler();
              q.find(".fk-timeVauleWrap").hide();
              if (r.$input) {
                r.$input.datetimepicker("hide")
              }
            });
            return q
          },
          options: function(i, k, j, m) {
            var l = {}
              , n = k.children(".fk-timepicker-time");
            if (typeof (j) == "string") {
              if (m === undefined) {
                return n.data(j)
              }
          ...
          value: function(i, k, l) {
            var m = k.find(".fk-timepicker-select"), j;
            if (l !== undefined) {
              if (k.hasClass("ui_tpicker_hour_slider")) {
                if (l >= 10) {
                  j = l + "：00"
                } else {
                  j = "0" + l + "：00"
            ...
            return j ? m.val(j) : m.val()
          }
        },
        TimePicker2: {
          create: function(n, m, q, i, l, p, j) {
            m.parents(".ui-timepicker-div").addClass("ui-timepicker2");
            var o = g("<input class='ui-timepicker2-input'/>");
            var k = this._timePanel;
            k.destroy();
          ...
          options: function(i, k, j, l) {
            this.value(i, k, this.value(i, k))
          },
          value: function(i, j, k) {
            if (k !== undefined) {
              return j.val(this._fillVal(k))
            }
            k = j.val();
            return isNaN(k) || k == "" ? "00" : k
          },
          _fillVal: function(i) {
          ...
          _timePanel: {
            ...
            create: function(m) {
              ...
              var n = this
                , q = m.hour_slider.val()
                , o = m.minute_slider.val()
                , l = m.second_slider.val()
                , r = this.$el = g("<div class='ui-timepicker2-panel'/>")
                , i = g("<div class='ui-timepicker2-spinner'/>").appendTo(r)
                , k = g("<div class='ui-timepicker2-footer'/>").appendTo(r)
                , j = g("<a href='javascript:void(0)' class='ui-timepicker2-cancel-btn'>取消</a>").appendTo(k)
                , p = g("<a href='javascript:void(0)' class='ui-timepicker2-sure-btn'>确定</a>").appendTo(k);
              g(".ui-timepicker2").append(r);
              g("#ui-datepicker-div").mousedown(function() {
                n.close()
              });
              r.mousedown(function(s) {
                s.stopPropagation()
              });
              p.click(function() {
                n.close()
              ...
              j.click(function() {
                n.close();
                m.hour_slider.val(q);
                m.minute_slider.val(o);
                m.second_slider.val(l);
                m._onTimeChange();
                m._onSelectHandler()
              });
              this.$el.fadeIn(300);
              this.set_hour = this._createScrollBar(i, {
                val: q,
                min: 0,
                max: 23,
                step: 1
                ...
                this.set_minute = this._createScrollBar(i, {
                  val: o,
                  min: 0,
                  max: 59,
                  step: 1
                }, function(s) {
                  m.minute_slider.val(s);
                  m._onTimeChange();
                  m._onSelectHandler()
                }),
                this.set_second = this._createScrollBar(i, {
                  val: l,
                  min: 0,
                  max: 59,
                  step: 1
                }, function(s) {
                  m.second_slider.val(s);
                  m._onTimeChange();
                  m._onSelectHandler()
                });
              return r
            ...
            _createScrollBar: function(r, s, k) {
              var o = this;
              var p = "";
              for (var m = s.min; m <= s.max; m++) {
                p += "<li>" + o._fillVal(m) + "</li>"
              }
              var q = g("<div class='ui-timepicker2-scrollbar'><ul class='ui-timepicker2-scrollbar-inner'>" + p + "</ul></div>");
              r.append(q);
              var j = 1;
              var n = q.find("li");
              q.mousewheel(function(i) {
                ...
                k(o._fillVal(s.val));
                n.removeClass("ui-timepicker2-active").eq(s.val).addClass("ui-timepicker2-active")
              });
              ...
              l();
              function l(i) {
                i || k(o._fillVal(s.val));
                q.scrollTop(s.val * 39);
                n.removeClass("ui-timepicker2-active").eq(s.val).addClass("ui-timepicker2-active")
              }
              return function(i) {
                s.val = i;
    ...
    g.fn.extend({
      timepicker: function(j) {
        ...
        return g(this).each(function() {
          g.fn.datetimepicker.apply(g(this), i)
        })
      },
      datetimepicker: function(j) {
        j = j || {};
        var i = arguments;
        if (typeof (j) == "string") {
          if (j == "getDate") {
            return g.fn.datepicker.apply(g(this[0]), i)
          } else {
            return this.each(function() {
              var k = g(this);
              k.datepicker.apply(k, i)
            })
          }
        } else {
          return this.each(function() {
            var k = g(this);
            k.datepicker(g.timepicker._newInst(k, j)._defaults)
          })
        }
      }
    });
    g.datepicker.parseDateTime = function(j, o, i, n, m) {
      var k = d(j, o, i, n, m);
      if (k.timeObj) {
        var l = k.timeObj;
        k.date.setHours(l.hour, l.minute, l.second, l.millisec)
      }
      return k.date
    }
    ;
    g.datepicker.parseTime = function(p, i, u) {
      var j = function(v, o) {
        var w = [];
        if (v) {
          g.merge(w, v)
        ...
        w = g.map(w, function(x) {
          return x.replace(/[.*+?|()\[\]{}\\]/g, "\\$&")
        });
        return "(" + w.join("|") + ")?"
      };
      var l = function(x) {
        var w = x.toLowerCase().match(/(h{1,2}|m{1,2}|s{1,2}|l{1}|t{1,2}|z|'.*?')/g)
          , v = {
          h: -1,
          m: -1,
          s: -1,
        ...
        if (w) {
          for (var o = 0; o < w.length; o++) {
            if (v[w[o].toString().charAt(0)] == -1) {
              v[w[o].toString().charAt(0)] = o + 1
            }
          }
        }
        return v
      };
      var k = b(b({}, g.timepicker._defaults), u || {});
      var t = "^" + p.toString().replace(/(hh?|mm?|ss?|[tT]{1,2}|[lz]|'.*?')/g, function(o) {
        switch (o.charAt(0).toLowerCase()) {
          case "h":
            return "(\\d?\\d)";
          case "m":
            return "(\\d?\\d)";
          ...
          case "z":
            return "(z|[-+]\\d\\d:?\\d\\d|\\S+)?";
          case "t":
            return j(k.amNames, k.pmNames);
          default:
            return "(" + o.replace(/\'/g, "").replace(/(\.|\$|\^|\\|\/|\(|\)|\[|\]|\?|\+|\*)/g, function(v) {
              return "\\" + v
            }) + ")?"
        }
      }).replace(/\s/g, "\\s?") + k.timeSuffix + "$", m = l(p), r = "", s;
      s = i.match(new RegExp(t,"i"));
      var q = {
        hour: 0,
        minute: 0,
        second: 0,
      ...
      if (s) {
        if (m.t !== -1) {
          if (s[m.t] === undefined || s[m.t].length === 0) {
            r = "";
            q.ampm = ""
          } else {
            r = g.inArray(s[m.t].toUpperCase(), k.amNames) !== -1 ? "AM" : "PM";
            q.ampm = k[r == "AM" ? "amNames" : "pmNames"][0]
          }
        }
        if (m.h !== -1) {
          if (r == "AM" && s[m.h] == "12") {
        ...
        if (m.m !== -1) {
          q.minute = Number(s[m.m])
        }
        if (m.s !== -1) {
          q.second = Number(s[m.s])
        }
        if (m.l !== -1) {
          q.millisec = Number(s[m.l])
        }
        if (m.z !== -1 && s[m.z] !== undefined) {
          var n = s[m.z].toUpperCase();
          switch (n.length) {
            case 1:
              n = k.timezoneIso8601 ? "Z" : "+0000";
              break;
            case 5:
              if (k.timezoneIso8601) {
                n = n.substring(1) == "0000" ? "Z" : n.substring(0, 3) + ":" + n.substring(3)
              }
              break;
            case 6:
              if (!k.timezoneIso8601) {
                n = n == "Z" || n.substring(1) == "00:00" ? "+0000" : n.replace(/:/, "")
              } else {
                if (n.substring(1) == "00:00") {
                  n = "Z"
                }
              }
              break
    ...
    ;
    g.datepicker.formatTime = function(m, l, k) {
      k = k || {};
      k = g.extend({}, g.timepicker._defaults, k);
      l = g.extend({
        hour: 0,
        minute: 0,
        second: 0,
      ...
      n = n.replace(/(?:hh?|mm?|ss?|[tT]{1,2}|[lz]|'.*?')/g, function(o) {
        switch (o.toLowerCase()) {
          ...
          case "h":
            return i;
          case "mm":
            return ("0" + l.minute).slice(-2);
          case "m":
            return l.minute;
          case "ss":
            return ("0" + l.second).slice(-2);
          case "s":
            return l.second;
          case "l":
            return ("00" + l.millisec).slice(-3);
          case "z":
            return l.timezone === null ? k.defaultTimezone : l.timezone;
          case "t":
          case "tt":
            if (k.ampm) {
              if (o.length == 1) {
              ...
              return o.charAt(0) === "T" ? j.toUpperCase() : j.toLowerCase()
            }
            return "";
          default:
            return o.replace(/\'/g, "") || "'"
    ...
    ;
    g.datepicker._base_selectDate = g.datepicker._selectDate;
    g.datepicker._selectDate = function(l, i) {
      var k = this._getInst(g(l)[0])
        , j = this._get(k, "timepicker");
      if (j) {
        j._limitMinMaxDateTime(k, true);
        k.inline = k.stay_open = true;
        this._base_selectDate(l, i);
        k.inline = k.stay_open = false;
    ...
    ;
    g.datepicker._base_updateDatepicker = g.datepicker._updateDatepicker;
    g.datepicker._updateDatepicker = function(l) {
      var i = l.input[0];
      if (g.datepicker._curInst && g.datepicker._curInst != l && g.datepicker._datepickerShowing && g.datepicker._lastInput != i) {
        return
      }
      if (typeof (l.stay_open) !== "boolean" || l.stay_open === false) {
        this._base_updateDatepicker(l);
        var k = this._get(l, "timepicker");
        if (k) {
          k._addTimePicker(l);
          l.dpDiv.off("click.time").on("click.time", function(n) {
            var m = l.dpDiv.find(".ui_tpicker_hour .fk-timeVauleWrap");
            if (!m.is(":visible") && g(n.target).hasClass("fk-timeVauleWrap") || g(n.target).hasClass("fk-timepicker-select")) {
              if (g(".ui_tpicker_hour .mCustomScrollbar").length < 1) {
                g(".ui_tpicker_hour .fk-timeVauleWrap").mCustomScrollbar({
                  theme: "fk-gray",
                  advanced: {
            ...
            m.hide();
            l.dpDiv.find(".fk-timepicker-select").removeClass("f-select").attr("_showTime", 0)
          });
          if (k._defaults.useLocalTimezone) {
            var j = new Date(l.selectedYear,l.selectedMonth,l.selectedDay,12);
            h(k, j);
            k._onTimeChange()
          }
        }
    ...
    ;
    g.datepicker._base_doKeyPress = g.datepicker._doKeyPress;
    g.datepicker._doKeyPress = function(n) {
      var o = g.datepicker._getInst(n.target)
        , l = g.datepicker._get(o, "timepicker");
      if (l) {
        if (g.datepicker._get(o, "constrainInput")) {
          var k = l._defaults.ampm
            , j = g.datepicker._possibleChars(g.datepicker._get(o, "dateFormat"))
            , i = l._defaults.timeFormat.toString().replace(/[hms]/g, "").replace(/TT/g, k ? "APM" : "").replace(/Tt/g, k ? "AaPpMm" : "").replace(/tT/g, k ? "AaPpMm" : "").replace(/T/g, k ? "AP" : "").replace(/tt/g, k ? "apm" : "").replace(/t/g, k ? "ap" : "") + " " + l._defaults.separator + l._defaults.timeSuffix + (l._defaults.showTimezone ? l._defaults.timezoneList.join("") : "") + (l._defaults.amNames.join("")) + (l._defaults.pmNames.join("")) + j
            , m = String.fromCharCode(n.charCode === undefined ? n.keyCode : n.charCode);
          return n.ctrlKey || (m < " " || !j || i.indexOf(m) > -1)
        }
      }
      return g.datepicker._base_doKeyPress(n)
    }
    ;
    g.datepicker._base_updateAlternate = g.datepicker._updateAlternate;
    g.datepicker._updateAlternate = function(o) {
      var p = this._get(o, "timepicker");
      if (p) {
        var i = p._defaults.altField;
        if (i) {
          var m = p._defaults.altFormat || p._defaults.dateFormat
            , j = this._getDate(o)
            , k = g.datepicker._getFormatConfig(o)
            , q = ""
            , r = p._defaults.altSeparator ? p._defaults.altSeparator : p._defaults.separator
            , n = p._defaults.altTimeSuffix ? p._defaults.altTimeSuffix : p._defaults.timeSuffix
            , l = p._defaults.altTimeFormat !== undefined ? p._defaults.altTimeFormat : p._defaults.timeFormat;
          q += g.datepicker.formatTime(l, p, p._defaults) + n;
          if (!p._defaults.timeOnly && !p._defaults.altFieldTimeOnly) {
            if (p._defaults.altFormat) {
              q = g.datepicker.formatDate(p._defaults.altFormat, (j === null ? new Date() : j), k) + r + q
            } else {
              q = p.formattedDate + r + q
            }
          }
          g(i).val(q)
        }
      } else {
        g.datepicker._base_updateAlternate(o)
      }
    }
    ;
    g.datepicker._base_doKeyUp = g.datepicker._doKeyUp;
    g.datepicker._doKeyUp = function(k) {
      var l = g.datepicker._getInst(k.target)
        , i = g.datepicker._get(l, "timepicker");
      if (i) {
        if (i._defaults.timeOnly && (l.input.val() != l.lastVal)) {
          try {
            g.datepicker._updateDatepicker(l)
          } catch (j) {
            g.datepicker.log(j)
          }
        }
      }
      return g.datepicker._base_doKeyUp(k)
    }
    ;
    g.datepicker._disableTimepickerDatepicker = function(k) {
      var j = this._getInst(k);
      if (!j) {
        return
      }
      var i = this._get(j, "timepicker");
      g(k).datepicker("getDate");
      if (i) {
        i._defaults.showTimepicker = false;
        i._updateDateTime(j)
      }
    }
    ;
    g.datepicker._enableTimepickerDatepicker = function(k) {
      var j = this._getInst(k);
      if (!j) {
        return
      }
      ...
      g(k).datepicker("getDate");
      if (i) {
        i._defaults.showTimepicker = true;
        i._addTimePicker(j);
        i._updateDateTime(j)
      }
    }
    ;
    g.datepicker._setTimeDatepicker = function(m, k, j) {
      var l = this._getInst(m);
      if (!l) {
        return
      }
      var i = this._get(l, "timepicker");
      if (i) {
        this._setDateFromField(l);
        var n;
        if (k) {
          if (typeof k == "string") {
            i._parseTime(k, j);
            n = new Date();
            n.setHours(i.hour, i.minute, i.second, i.millisec)
          } else {
            n = new Date(k.getTime())
          }
    ...
    ;
    g.datepicker._base_setDateDatepicker = g.datepicker._setDateDatepicker;
    g.datepicker._setDateDatepicker = function(k, i) {
      var j = this._getInst(k);
      if (!j) {
        return
      }
      var l = (i instanceof Date) ? new Date(i.getTime()) : i;
      this._updateDatepicker(j);
      this._base_setDateDatepicker.apply(this, arguments);
      this._setTimeDatepicker(k, l, true)
    }
    ;
    g.datepicker._base_getDateDatepicker = g.datepicker._getDateDatepicker;
    g.datepicker._getDateDatepicker = function(m, k) {
      var l = this._getInst(m);
      if (!l) {
        return
      }
      var j = this._get(l, "timepicker");
      if (j) {
        if (l.lastVal === undefined) {
          this._setDateFromField(l, k)
        }
        var i = this._getDate(l);
        if (i && j._parseTime(g(m).val(), j.timeOnly)) {
          i.setHours(j.hour, j.minute, j.second, j.millisec)
        }
        return i
      }
      return this._base_getDateDatepicker(m, k)
    }
    ;
    g.datepicker._base_parseDate = g.datepicker.parseDate;
    g.datepicker.parseDate = function(m, l, j) {
      var i;
      try {
        i = this._base_parseDate(m, l, j)
      } catch (k) {
        i = this._base_parseDate(m, l.substring(0, l.length - (k.length - k.indexOf(":") - 2)), j)
      }
      return i
    }
    ;
    g.datepicker._base_formatDate = g.datepicker._formatDate;
    g.datepicker._formatDate = function(l, i, m, k) {
      var j = this._get(l, "timepicker");
      if (j) {
        j._updateDateTime(l);
        return j.$input.val()
      }
      return this._base_formatDate(l, i, m, k)
    }
    ;
    g.datepicker._base_optionDatepicker = g.datepicker._optionDatepicker;
    g.datepicker._optionDatepicker = function(o, j, r) {
      var m = this._getInst(o), k;
      if (!m) {
        return null
      }
      var n = this._get(m, "timepicker");
      if (n) {
        var l = null, q = null, p = null, s = n._defaults.evnts, t = {}, i;
        if (typeof j == "string") {
          if (j === "minDate" || j === "minDateTime") {
            l = r
          } else {
            if (j === "maxDate" || j === "maxDateTime") {
              q = r
            } else {
              if (j === "onSelect") {
                p = r
        ...
        if (l) {
          ...
          if (q) {
            ...
            n._defaults.maxDate = q;
            n._defaults.maxDateTime = q
          } else {
            if (p) {
              n._defaults.onSelect = p
            }
          }
        }
      }
      if (r === undefined) {
        return this._base_optionDatepicker.call(g.datepicker, o, j)
      }
      return this._base_optionDatepicker.call(g.datepicker, o, k || j, r)
    }
    ;
    function f(i) {
      var j;
    ...
    var e = function(k, j, q, r) {
      try {
        var p = r && r.separator ? r.separator : g.timepicker._defaults.separator
          , v = r && r.timeFormat ? r.timeFormat : g.timepicker._defaults.timeFormat
          , s = r && r.ampm ? r.ampm : g.timepicker._defaults.ampm
          , u = v.split(p)
          , n = u.length
          , m = j.split(p)
          , o = m.length;
        if (!s) {
          u = g.trim(v.replace(/t/gi, "")).split(p);
          n = u.length
        }
        if (o > 1) {
          return [m.splice(0, o - n).join(p), m.splice(0, n).join(p)]
        }
      } catch (l) {
        if (l.indexOf(":") >= 0) {
          var t = j.length - (l.length - l.indexOf(":") - 2)
            , i = j.substring(t);
          return [g.trim(j.substring(0, t)), g.trim(j.substring(t))]
        } else {
          throw l
        }
      }
    ...
    var d = function(k, m, j, n, p) {
      var l;
      var o = e(k, j, n, p);
      l = g.datepicker._base_parseDate(k, o[0], n);
      if (o[1] !== "") {
        var i = o[1]
          , q = g.datepicker.parseTime(m, i, p);
        if (q === null) {
    ...
    var h = function(l, k) {
      if (l && l.timezone_select) {
        l._defaults.useLocalTimezone = true;
        var j = typeof k !== "undefined" ? k : new Date();
        var i = g.timepicker.timeZoneOffsetString(j);
        if (l._defaults.timezoneIso8601) {
          i = i.substring(0, 3) + ":" + i.substring(3)
        }
        l.timezone_select.val(i)
      }
    };
    g.timepicker = new c();
    g.timepicker.timeZoneOffsetString = function(j) {
      var l = j.getTimezoneOffset() * -1
        , k = l % 60
        , i = (l - k) / 60;
      return (l >= 0 ? "+" : "-") + ("0" + (i * 101).toString()).substr(-2) + ("0" + (k * 101).toString()).substr(-2)
    }
    ;
    g.timepicker.timeRange = function(k, i, j) {
      return g.timepicker.handleRange("timepicker", k, i, j)
    }
    ;
    g.timepicker.dateTimeRange = function(k, i, j) {
      g.timepicker.dateRange(k, i, j, "datetimepicker")
    }
    ;
    g.timepicker.dateRange = function(k, i, j, l) {
      l = l || "datepicker";
      g.timepicker.handleRange(l, k, i, j)
    }
    ;
    g.timepicker.handleRange = function(n, l, i, j) {
      g.fn[n].call(l, g.extend({
      ...
      if (n != "timepicker" && j.reformat) {
        g([l, i]).each(function() {
          var p = g(this)[n].call(g(this), "option", "dateFormat")
            , o = new Date(g(this).val());
          if (g(this).val() && o) {
            g(this).val(g.datepicker.formatDate(p, o))
          }
        })
      }
      m(l, i, l.val());
      function m(q, o, p) {
        if (o.val() && (new Date(l.val()) > new Date(i.val()))) {
          o.val(p)
        }
      }
      k(l, i, "minDate");
      ...
      function k(r, o, q) {
        ...
        var p = g(r)[n].call(g(r), "getDate");
        if (p.getTime) {
          g(o)[n].call(g(o), "option", q, p)
        }
      }
      return g([l.get(0), i.get(0)])
    }
    ;
    g.timepicker.version = "1.0.5"
  }
)(jQuery);
jQuery(function(b) {
  b.timepicker.regional["zh-CN"] = {
    closeText: "关闭",
    prevText: "&#x3c;上月",
    nextText: "下月&#x3e;",
    currentText: "今天",
  ...
  b.timepicker.setDefaults(b.datepicker.regional["zh-CN"])
});
